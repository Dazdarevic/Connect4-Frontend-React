...
const ConnectFourEasy = () => {
  ...
  const getBestMove = useCallback(async (board) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/playground/get_best_move4/?board=${JSON.stringify(board)}`, {
        method: 'GET',
        headers: {
...
export default ConnectFourEasy;